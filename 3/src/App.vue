<template>
  <div id="app">
    <div :class="[$style.wrapper]"> <!--показывает что стиль применяется для модуля -->
   <header>
...
computed:{
  ...
  created(){
    this.paymentsList = this.fetchData() //функция fetchData возвращает массивБ и мы присваиваем её листу
  }
}
</script>
...
// Создать компоненты, которые будут отвечать за основной функционал:
...
// компонент формы добавления новой статьи расходов. Форма должна иметь 3 поля: описание платежа, сумма и дата. Все поля должны сохранять свое состояние в data компонента с помощью директивы v-model. В форме необходимо также сделать кнопку, по нажатию на которую будет происходить отправка введенных данных в родительский компонент. Отправку данных реализовать с помощью генерации события через метод $emit. ### 
// 3. Созданные компоненты подключить в корневой компонент приложения: <App />. 
// Компонент App.vue должен создаться автоматически на шаге 
// 1. Если вы создаете приложение не с помощью Vue CLI, то создайте App.vue вручную. Далее нужно связать подключенные компоненты в единую логику:
